...
def download_frp():
    ...
    """
    frpc_path = os.path.join(PROJECT_ROOT, "frpc.exe")
    ...
    try:
        ...
        with zipfile.ZipFile(io.BytesIO(zip_data)) as zf:
            ...
            zf.extract(frpc_member, PROJECT_ROOT)
...
def setup_frp_and_keep_alive(port=8000, frp_config=None, stop_event=None):
    ...
    # Validate frp_config: only allow config files within the project root directory
    resolved_config = os.path.realpath(os.path.join(PROJECT_ROOT, frp_config))
    if not resolved_config.startswith(os.path.realpath(PROJECT_ROOT) + os.sep):
